...
window.fbSetPresence = async function(user, isMestre){
  ...
  const _handleExit = () => {
    ...
    // Grava lastSeen via fetch keepalive (funciona em beforeunload)
    ...
    // Remove presença via fetch keepalive (sendBeacon não suporta DELETE)
    fetch(b + '/rest/v1/mesa_state?or=(path.eq.' + encodeURIComponent('presence/' + user) + ')', {
      method: 'DELETE',
      keepalive: true,
      headers: { ..._sbHeaders() }
    }).catch(()=>{});
...
window.fbRemovePresence = async function(user, since){
  ...
  // isMestre é variável global do app — usar window para garantir acesso correto
  const _isMestreVal = (typeof isMestre !== 'undefined') ? !!isMestre : false;
  await _sbPut('last_seen/' + user, { user, isMestre: _isMestreVal, lastSeen: Date.now(), sessionDuration });
...
window.fbSaveMap = function(mapKey, mapData){
  ...
  if((typeof draggingToken !== 'undefined' && draggingToken) || (typeof draggingStruct !== 'undefined' && draggingStruct)){
    const now = Date.now();
...
function saveFirebaseConfig(){
  ...
  // Inicializa o cliente Realtime já agora (antes do reload)
  try {
  ...
  const doSync = () => {
    ...
    setTimeout(()=>{
      window.fbWatchDB(remote => {
        ...
        // Salva bg do mapa compartilhado se mudou e não sou o Mestre
        if(!isMestre && remote.mapbg && remote.mapbg !== (db.maps&&db.maps['shared'])){
          if(!db.maps) db.maps={};
          db.maps['shared'] = remote.mapbg;
          // Guarda para quando o mapa for aberto; aplica se função existir
          window._pendingRemoteBg = remote.mapbg;
          if(typeof applyRemoteBg === 'function') applyRemoteBg(remote.mapbg);
        }
        localStorage.setItem(DB_KEY, JSON.stringify(db));
...
function _doFbConnect(user, isMestreFlag){
  ...
  if(window.fbWatchMap){
    window.fbWatchMap(data => {
      ...
      try{
        ...
        // Aplica no canvas se o mapa já foi inicializado (mapCtx definido pelo módulo de mapa)
        if(typeof mapCtx === 'undefined' || !mapCtx) return;
        // Tokens: se estou arrastando o meu agora, preserva posição local
        const myTok = (typeof draggingToken !== 'undefined' && draggingToken && !isMestreFlag)
        ...
        if(typeof draggingStruct === 'undefined' || !draggingStruct) mapStructures = newStructs;
  ...
  if(window.fbWatchBg){
    ...
    window.fbWatchBg(data=>{
      ...
      // Aplica no canvas se disponível, ou guarda para quando o mapa abrir
      window._pendingRemoteBg = data.data;
      if(typeof applyRemoteBg === 'function') applyRemoteBg(data.data);
    });
  }
...
checkFirebaseSetup();
// Inicializa o cliente Supabase JS (para Realtime/WebSocket) se a config estiver disponível
(function _initSupabaseJSClient(){
  try {
    const cfg = _sbCfg();
    if(cfg && cfg.url && cfg.anonKey && window.supabase && window.supabase.createClient){
      window._supabaseClient = window.supabase.createClient(cfg.url, cfg.anonKey, {
        realtime: { params: { eventsPerSecond: 10 } }
      });
      console.log('[Supabase] Cliente JS inicializado — Realtime ativo.');
    }
  } catch(e) {
    console.warn('[Supabase] Não foi possível inicializar cliente JS (Realtime desabilitado):', e);
  }
})();
loadDB();
